...
PwmView::PwmView()
    : callback(nullptr), callback_context(nullptr), channel_item(nullptr),
      freq_item(nullptr), duty_item(nullptr) {
  view = view_alloc();
  view_set_context(view, this);
  view_allocate_model(view, ViewModelTypeLocking, sizeof(PwmViewModel));
  with_view_model_cpp(
      view, PwmViewModel *, model,
      {
        model->menu = new Menu();
        setup_menu(model->menu);
      },
      true);
...
void PwmView::setup_menu(Menu *menu) {
  // Channel selection
...
bool PwmView::input_callback(InputEvent *event, void *context) {
  ...
  bool result;
  with_view_model_cpp(
      pwm->view, PwmViewModel *, model,
      { result = model->menu->handle_input(event, pwm->view); }, result);
  return result;
}
