...
public:
    ...
    // Add a menu item
    bool add_item(std::unique_ptr<MenuItem> item) {
        if(items_count_ >= MAX_ITEMS) {
            return false;
        }
        items_[items_count_++] = std::move(item);
    ...
    bool handle_input(InputEvent* event) {
        if(items_count_ == 0) return false;
    ...
    MenuItem* get_selected_item() {
        if(selected_index_ >= 0 && selected_index_ < (int)items_count_) {
            return items_[selected_index_].get();
        }
        return nullptr;
    }

private:
    std::unique_ptr<MenuItem> items_[MAX_ITEMS];
    size_t items_count_;
    int selected_index_;
};
